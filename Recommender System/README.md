...
本项目模拟电商 / 视频平台的后台推荐系统，使用 **MovieLens 100k** 数据集  
（`u.user`, `u.item`, `u.data`），从零实现工业界常见的 **“召回 + 排序”两阶段推荐架构**。
...
在课堂或答辩里，可以按照下面顺序进行讲解：

1. **业务背景**：从“用户刷视频 / 逛电商”引入推荐系统的必要性  
2. **两阶段架构**：为什么要“召回 + 排序”，类比“先大致筛一批，再精细排序”  
3. **Item-based CF 的直观理解**：
   - “喜欢《星球大战》的人也喜欢《星际迷航》”的故事  
   - 展示余弦相似度公式和评分矩阵示意图  
4. **排序模型的构造方式**：
   - 正负样本如何采样  
   - 为何要把“用户特征 + 电影特征”拼在一起  
...
5. **冷启动策略**：
   - 说明“新用户没有历史行为”的困难  
   - 使用“热门 Top-5 电影兜底”的简单但有效策略  
6. **离线评估指标 Precision@5**：
   - 以具体用户示例说明“推荐命中率”的直观含义  
7. **代码结构**：
   - 强调项目解耦良好，每个模块单一职责、易于维护与扩展  

这样可以从“业务 – 算法 – 实现 – 评估”四个层面完整展示本项目。
